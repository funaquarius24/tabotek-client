...
export default function AdminMediaPage() {
  ...
  const { data, isLoading, isError } = useFiles({ page, limit: pageSize, type: typeParam, search: search || undefined });
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-gray-600 mt-2">Upload and manage images, documents, and other media files.</p>
        </div>
        <button onClick={() => setShowUpload(!showUpload)} className="px-6 py-3 bg-blue-600 hover:bg-blue-700 text-white font-medium rounded-lg transition-colors">
      ...
      {showUpload && (
        <div className="bg-white rounded-xl shadow-md p-6">
          <Upload onSuccess={() => { setShowUpload(false); setPage(1); }} accept="image/*,video/*,.pdf,.zip,.gz,.csv,.txt" maxSize={10 * 1024 * 1024} />
        </div>
      )}
      ...
      <div className="bg-white rounded-xl shadow-md p-6">
        <div className="flex items-center justify-between mb-6">
          ...
          <div className="flex items-center gap-3">
            <input type="text" value={search} onChange={e => { setSearch(e.target.value); setPage(1); }} placeholder="Search files..." className="px-4 py-2 border border-gray-300 rounded-lg text-sm w-48" />
            <div className="flex gap-2">
              {(['all', 'image', 'video', 'document'] as FilterType[]).map(f => (
                <button key={f}
                  className={`px-4 py-2 rounded-lg font-medium transition-colors ${filter === f ? 'bg-blue-600 text-white' : 'bg-gray-100 text-gray-800 hover:bg-gray-200'}`}
              ...
              {files.map((file: FileRecord) => (
                <div key={file._id} className="group relative aspect-square overflow-hidden rounded-lg bg-gray-100">
                  ...
                  <div className="absolute inset-0 bg-black/0 group-hover:bg-black/30 transition-all flex items-center justify-center opacity-0 group-hover:opacity-100">
                    <div className="flex gap-2">
                      <button className="p-2 bg-white rounded-lg shadow-sm hover:shadow-md" onClick={() => navigator.clipboard.writeText(file.url)} title="Copy URL">📋</button>
                      <button className="p-2 bg-red-100 text-red-800 rounded-lg shadow-sm hover:shadow-md" onClick={() => handleDelete(file._id)} title="Delete">🗑️</button>
                    </div>
                  </div>
